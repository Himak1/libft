...
/*   Updated: 2021/03/27 10:54:41 by jhille        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	reststore(int fd, char *store[], char *line, int i)
{
	...
	if (store[fd] == 0)
	{
		store[fd] = ft_gnl_substr(line, i, linelen(line, 2));
		if (store[fd] == 0)
			return (-1);
		else
			return (1);
	...
	if (i > 0)
		temp = ft_gnl_substr(line, i, linelen(line, 2) - i);
	else
		temp = ft_gnl_substr(store[fd], i * -1, linelen(store[fd], 2) - (i * -1));
	free(store[fd]);
	if (temp == 0)
		return (-1);
	store[fd] = temp;
...
static int	finishline(int fd, char *buf, char *store[], char **line)
{
	...
	ret = 1;
	*line = ft_gnl_substr(store[fd], 0, linelen(store[fd], 1));
	if (*line == 0)
		return (-1);
	...
	{
		temp = ft_gnl_substr(*line, 0, linelen(*line, 1));
		free(*line);
		*line = temp;
	}
	return (ret);
...
static int	writeline(int fd, char *buf, char *store[], char **line)
{
	...
	if (buf[0] == '\0')
		...
		*line = ft_gnl_substr(buf, 0, linelen(buf, 1));
	if (*line == 0)
		return (-1);
